...
export const validateFileUpload = (file: File): FileValidationResult => {
    ...
    if (file.size > FILE_UPLOAD_CONFIG.MAX_SIZE_BYTES) {
        return {
            ...
            error: {
                code: "FILE_TOO_LARGE",
                message: `File exceeds maximum size of ${FILE_UPLOAD_CONFIG.MAX_SIZE_MB} MB. Current size: ${(file.size / (1024 * 1024)).toFixed(2)} MB`,
                fileName: file.name,
            },
        };
    }
    ...
    if (!FILE_UPLOAD_CONFIG.ALLOWED_MIME_TYPES.includes(file.type as any)) {
        return {
            ...
            error: {
                code: "INVALID_TYPE",
                message: `File type not supported. Allowed types: ${FILE_UPLOAD_CONFIG.ALLOWED_EXTENSIONS.join(", ")}`,
                fileName: file.name,
            },
        };
    }
